...
async function main() {
  ...
  for (const [name, sessions, adapter] of [
    ...
    const t0 = performance.now()
    await adapter.readSession(sessions[0].id)
    console.log(`${name} readSession(1): ${Math.round(performance.now() - t0)} ms`)
  }
  ...
  console.log(`codex readSession avg (${sampleCount} samples): ${Math.round(avg)} ms`)
  console.log(
    `estimated codex read all: ${Math.round((avg * codex.length) / 1000)} sec for ${codex.length} sessions`
  )
